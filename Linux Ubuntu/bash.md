...
명령 실행 결과를 변수화한다. 명령 실행 결과를 변수에 저장하거나 다른 명령의 매개 변수로 넘겨줄 때 사용한다. 또는 문자열안에 명령의 실행 결과를 넣을 때 사용한다.

```bash
# 도커의 실행중인 프로세스를 전부 가져와서 컨테이너 아이디를 출력하고 삭제한다
...
한 줄에서 명령을 여러 개 실행한다. 앞에서 실행한 명령이 에러 없이 실행되어야 뒤에 오는 명령어도 정상적으로 실행된다.
...
>
> 소스 코드를 빌드합니다  
> 컴파일러는 코드를 컴파일 하지만, 대부분의 경우 독립적으로 하지는 못하고 외부 라이브러리가 있어야 합니다  
> 이 단계가 끝나면 설치하려는 어플리케이션의 실행파일이 만들어집니다
>
> make install
>
> 어플리케이션 파일에 필요한 모든것을 시스템 디렉토리로 이동합니다  
> 어플리케이션의 실행 파일이 작성되어 나중에 사용될 수 있도록 적절한 시스템 디렉토리로 이동할 수 있기 때문에 make후에 수행해야합니다

## ;

한 줄에서 명령을 여러 개 실행한다. 앞 명령이 실패해도 뒤에 오는 명령을 실행한다
...
' ' 안에 들어있는 변수는 처리되지 않고 변수명 그대로 사용된다. ``와 \$()도 처리되지 않고 그대로 사용된다.

```bash
# 변수 user의 변수명인 $user 그대로 출력한다
echo '$user'
$suer    # 출력 값
